...
router.post("/addingredientinrecipe", async (req, res) => {
 ...
 const data = await db.query('SELECT a.recipeName, b.ingredientName FROM recipe a INNER JOIN ingredientinrescipe c ON a.id = c.recipeId INNER JOIN ingredient b ON b.id = c.ingredientId WHERE a.recipeName = $1 AND b.ingredientName = $2;', [recipename, ingredientname]);
 ...
 if(data.rows.length !== 0) {
    ...
    try {
        const result = await db.query("INSERT INTO ingredientinrescipe (recipeid, ingredientid) SELECT a.id, b.id FROM recipe a JOIN ingredient b ON a.recipeName = $1 AND b.ingredientname = $2;", [recipename, ingredientname]); 
        console.log(result.rowCount);
